...
def evaluate_dataset(
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader):
            ...
            if eval_type == "multiple_choice":
                ...
                batch["answer_choices_scores"] = (
                    answer_choice_scores.cpu().numpy().tolist()
                )
                ...
                (generated_ids, generated_txt) = merge_method.generate(batch)
    ...
    if metric == "accuracy":
        ...
        score = accuracy.compute(
            references=[example["label"] for example in all_batches],
            predictions=[example["predicted_choice"] for example in all_batches],
        )
    elif metric == "rouge":
...
def evaluate_model(
    ...
    for dataset_filepath, eval_type, metric in zip(
        all_dataset_filepaths, all_eval_types, all_metrics
    ):
        score, dataset_predictions = evaluate_dataset(
            merge_method, dataset_filepath, eval_type, metric
        )
