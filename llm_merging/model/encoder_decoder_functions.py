...
def compute_loss(transformer, tokenizer, batch):
    ...
    sum_negative_log_prob = torch.sum(negative_log_probs * target_mask)

    loss = sum_negative_log_prob / torch.sum(target_mask)

    return loss


def predict_multiple_choice(transformer, tokenizer, batch):
    tokenized_batch = tokenize_batch(tokenizer, batch, transformer.device)
    ...
    answer_choice_log_probs = torch.sum(
        answer_choices_ids_log_probs * answer_choices_mask, dim=2
    )
...
def generate(transformer, tokenizer, batch, max_gen_len):
    tokenized_batch = tokenize_batch(tokenizer, batch, transformer.device)
