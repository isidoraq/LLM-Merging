...
def predict_multiple_choice(transformer, input_tokenizer, target_tokenizer, batch):
    tokenized_batch = tokenize_batch(
        input_tokenizer, target_tokenizer, batch, transformer.device
    )
    output = transformer(
    ...
    expanded_input_mask = torch.repeat_interleave(
        tokenized_batch["input_mask"], num_answer_choices, dim=0
    )
    ...
    for pastKeyValues_perLayer in past_key_values:
        ...
        expanded_past_key_valyes.append(tuple(list_broadcast_pastKeyValues_perLayer))
    ...
    shifted_answer_choice_ids_logits = answer_choice_ids_logits[
        ..., :-1, :
    ].contiguous()
    ...
    answer_choice_log_probs = torch.sum(
        shifted_answer_choice_ids_log_probs * shifted_answer_choices_mask, dim=2
    )
...
def generate(transformer, input_tokenizer, target_tokenizer, batch, max_gen_len):
    tokenized_batch = tokenize_batch(
        input_tokenizer, target_tokenizer, batch, transformer.device
    )
...
def tokenize_batch(input_tokenizer, target_tokenizer, batch, device):
    ...
    keys_to_tokenize_with_tokenizer = [
        ("input", input_tokenizer),
        ("answer_choices", target_tokenizer),
