...
class Merges(object):
        ...
        if self.architecture == "encoder_decoder":
            ...
            if self.input_tokenizer is None or self.target_tokenizer is None:
                ...
                if self.input_tokenizer.pad_token_id is None:
                    self.input_tokenizer.pad_token_id = (
                        self.input_tokenizer.eos_token_id
                    )
    ...
    def predict_multiple_choice(self, batch):
        ...
        if self.architecture == "encoder_decoder":
            ...
            return encoder_decoder_functions.predict_multiple_choice(
                self.base_model, self.tokenizer, batch
            )
        elif self.architecture == "decoder":
            return decoder_functions.predict_multiple_choice(
                self.base_model, self.input_tokenizer, self.target_tokenizer, batch
            )
        else:
            raise NotImplementedError(
                f"Architecture not implemented {self.architecture}"
    ...
    def generate(self, batch):
        ...
        if self.architecture == "encoder_decoder":
            assert self.tokenizer is not None
            return encoder_decoder_functions.generate(
                self.base_model, self.tokenizer, batch, self.max_gen_len
            )
        elif self.architecture == "decoder":
            return decoder_functions.generate(
                self.base_model,
    ...
    def _load_huggingface_models_and_configs(self):
        ...
        for model_name, revision_id in self.list_models:
            ...
            peft_model_parameters = load_peft_weights(
                model_name, revision=revision_id, token=os.environ["HF_AUTH_TOKEN"]
            )
            ...
            if parameter_names != set(peft_model_parameters.keys()):
                print(
                    f"WARNING: parameters in {model_name} do not match {self.list_models[0]}"
                )
